...
from blip_captioning_pipeline import DEFAULT_WEIGHTS_DIR, WEIGHT_FILE, BlipCaptioningPipeline
        ...
        out.append(
            {
                ...
                "image": str(path),
                "captions": [
                    f"A {colour} {shape} on green grass under a blue sky.",
                    f"There is a large {colour} {shape} in a field.",
                    f"A drawing of a {colour} {shape} above the grass.",
                ],
            }
        )
    return out
...
def _captions(pipe, records):
    ...
    for r in records:
        with Image.open(r["image"]) as image:
            image.load()
            out.append(pipe.caption(image)["caption"])
    return out
...
def test_evaluate_scores_reference_captions(pipe, records):
...
def test_one_epoch_adaptation_and_artifact_round_trip(pipe, records, tmp_path):
    ...
    assert set(result["history"][1]["val"]) == {
        "bleu4",
        "rouge_l",
        "cider_d",
        "unigram_f1",
    ...
    reloaded = BlipCaptioningPipeline.from_artifact(artifact, device="cpu")
...
def test_load_artifact_refuses_a_tensor_set_that_differs_from_the_recorded_configuration(
    ...
    with pytest.raises(ValueError, match="does not match its recorded configuration"):
        BlipCaptioningPipeline.from_artifact(fewer, device="cpu")
    ...
    with pytest.raises(ValueError, match="tensor names differ"):
        BlipCaptioningPipeline.from_artifact(extra, device="cpu")
    ...
    with pytest.raises(ValueError, match="does not match its recorded configuration"):
        BlipCaptioningPipeline.from_artifact(other_layers, device="cpu")
...
def test_caption_adapt_and_reload_run_on_a_cuda_device(records, tmp_path):
    ...
    cuda = BlipCaptioningPipeline.from_pretrained(device="cuda:0")
    assert cuda.device == "cuda:0"
    with Image.open(records[0]["image"]) as image:
        image.load()
        first = cuda.caption(image)
    ...
    reloaded = BlipCaptioningPipeline.from_artifact(artifact, device="cuda:0")
