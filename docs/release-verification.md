...
|---|---|---|---|---|
| `blip_captioning_colab.ipynb` (`E2E`) | pending | — | Kaggle Tesla T4 (clean container, no repository checkout) | pending — the `E2E` blob has not executed in a supported runtime yet |
...
Facts a reviewer should still weigh: the frozen model is already a competent captioner on VizWiz photographs
(CIDEr-D ≈ 0.59 against 0.05 for the best constant caption), so the adaptation gain is small — a few hundredths of
CIDEr-D with BLEU-4 and ROUGE-L moving slightly the other way as the captions lengthen toward the corpus's style —
and the 70-photograph test split carries no dispersion estimate; a higher learning rate (5e-5) or twice the training
data at 2e-5 overfit within two epochs in the build record and the selector kept the frozen weights or lost
held-out CIDEr-D; the 40-photograph validation split makes epoch selection noisy; and the drawn scenes re-captioned
after adaptation are three images of evidence about behaviour outside the corpus, not a measurement.
