...
`tutorials/blip_captioning_colab.ipynb` (`TASK-INFERENCE`, **standalone** carrier) is a
...
- the standalone carrier (ST1–ST6, PAR1–PAR3): no clone, repository install or repository import on
  ...
  `src/blip_captioning_pipeline/pipeline.py` after the generator's documented rewrites; the
...
- the profile-specific public-API calls (`stage_missing_files`, `verify_snapshot`,
  `BlipCaptioningPipeline.from_pretrained(weights_dir=...)`, `validate_inputs`, `caption`,
  `keyword_hits`, `evaluation_report`), the ceiling print (`MIN_IMAGE_SIDE`, `MAX_IMAGE_SIDE`, `IMAGE_SIZE`,
  `MAX_PREFIX_CHARS`, `MAX_NEW_TOKENS`, `DEFAULT_MAX_NEW_TOKENS`, `DECODING`), the exports (including the
  executed-vs-hosted weight-file names), the learner-facing statements (caller-owned budget and prefix, no
  score in generated text, the `truncated` flag, the weight-format note, `not-measurable` without references,
  the model captions any image, capability exclusions) and the gated-off BYOD default listed in the
  validator; forbidden patterns (credential-in-URL, any `git clone` / `github.com` / repository import on
  the primary path, a mutable `revision='main'`, direct `from transformers import` /
  `BlipForConditionalGeneration` / `BlipProcessor` / `model.generate(` / `weights_only=` /
...
5. verify every default-path stage completes:
   ...
   - the carried module cell executes (defines `BlipCaptioningPipeline`, `validate_inputs`,
     `evaluation_report`, `keyword_hits`, `unigram_f1`, `verify_snapshot`, `stage_missing_files`) with no
     import of the repository package;
   - three synthetic cartoon scenes drawn in code with their RGB SHA-256 printed and the ceilings
     (`MIN_IMAGE_SIDE` 16, `MAX_IMAGE_SIDE` 4096, `IMAGE_SIZE` 384, `MAX_PREFIX_CHARS` 128,
     `MAX_NEW_TOKENS` 64, `DEFAULT_MAX_NEW_TOKENS` 30, `DECODING` greedy) surfaced;
   - pinned `Salesforce/blip-image-captioning-base` acquisition at the immutable revision through the
     carried module: the inline `MANIFEST` is asserted against the module identity and written to
     `weights/blip-image-captioning-base/`, `stage_missing_files(WEIGHTS_DIR, allow_download=True)` reports
     all 8 manifest entries on a clean runtime (`pytorch_model.bin` among them; `tf_model.h5` must not be
     fetched), `verify_snapshot` returns its summary dict, and `from_pretrained(weights_dir=WEIGHTS_DIR)`
     loads from the verified directory with no further Hub access (any download in the logs after staging
     is a finding);
   - `validate_inputs` writes `outputs/blip_captioning_input_manifest.json` (verdict `accepted`, three
   ...
   - `caption` returning one caption per image with `truncated` false on the default budget; record the
     captions (the card-pass CPU smoke produced `a red house with a tree and a ball`, `a beach scene with a
     sail and a sun`, `a red and yellow apple on a white background`; a materially different result is a
     finding to record, not a failure by itself, because no metric is asserted — greedy decoding on a
     different device can diverge) and the keyword observations;
   - `evaluation_report` writes `outputs/blip_captioning_evaluation_report.json` with verdict
   ...
   - `outputs/blip_captioning_result.json`, `outputs/blip_captioning_captions.csv` and
     `outputs/blip_captioning_annotated.png` written with `NOTEBOOK_SOURCE`, model revision, model
     licence, the executed and hosted weight-file names, runtime versions and device;
...
`git rev-parse <commit>:tutorials/blip_captioning_colab.ipynb`). Wall times, when recorded,
...
| Date (UTC) | Commit / notebook blob | Executor | Path exercised | Wall | Outcome |
|---|---|---|---|---|---|
| 2026-09-14 | `504cd92` / `88c86a876f02` | Kaggle CPU (`kurtvalcorza/dimer-nb2-blip-captioning` v1) | Default sample path | 242.6 s | **PASSED** — 8/8 ok code cells executed cleanly, 18 files, 991 MB staged |
...
repository never loads and whose equivalence to the pickle it has not verified; the tutorial samples are flat
cartoons, not photographs, and the model's miscount on them (an orange captioned as a second apple) is
recorded; on blank white images the model produced a striped rug and on uniform noise a colourful TV screen,
so an empty image yields a fluent invented caption with no signal; every image is resized to 384×384
without preserving aspect ratio; and each image costs an encoder pass (~0.2–0.5 s on the reference CPU), so
a long image list scales linearly.
